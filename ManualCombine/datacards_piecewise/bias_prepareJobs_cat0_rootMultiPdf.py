...
combine -m {mass} -M FitDiagnostics {base_path}window{window}_cat0.txt --expectSignal=0 --freezeParameters MH --saveWorkspace --X-rtd MINIMIZER_freezeDisassociatedParam --rMin -10 --rMax 10  --cminDefaultMinimizerStrategy 0 -t 1000 --toysFrequentist --toysFile {mass_path}higgsCombineFreeze{freeze}Cat0.GenerateOnly.mH{mass}.123456.root --keepFailures -n Freeze{freeze}Cat0W{window}
...
transfer_input_files = {executable_path},{base_path}window{window}_cat0.txt, {mass_path}higgsCombineFreeze{freeze}Cat0.GenerateOnly.mH{mass}.123456.root
...
for freeze in range(0,5):
  for i,mass in enumerate(mass_points):
    window=i+1
    mass_path = os.path.join(exe_path, "freezepdf"+str(freeze)+"_cat0/")
    ...
    with open(executable_path, "w") as f:
        f.write(executable_template.format(base_path=base_path,exe_path=exe_path, mass_path=mass_path, mass=mass, window=window, freeze=freeze))

    # Write submission file
    submission_path = os.path.join(mass_path, f"bias_study_W{window}.submit")
    with open(submission_path, "w") as f:
        f.write(submission_template.format(executable_path=executable_path, base_path=base_path, mass_path=mass_path, mass=mass, exe_path=exe_path, window=window, freeze=freeze))
