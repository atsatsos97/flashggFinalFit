...
combine -m {mass} -M FitDiagnostics {base_path}window{window}_cat1.txt --expectSignal=0 --freezeParameters MH --saveWorkspace --X-rtd MINIMIZER_freezeDisassociatedParam --rMin -10 --rMax 10  --cminDefaultMinimizerStrategy 0 -t 1000 --toysFrequentist --toysFile {mass_path}higgsCombineFreeze{freeze}Cat1.GenerateOnly.mH{mass}.123456.root --keepFailures -n Freeze{freeze}Cat1W{window}
...
transfer_input_files = {executable_path},{base_path}window{window}_cat1.txt, {mass_path}higgsCombineFreeze{freeze}Cat1.GenerateOnly.mH{mass}.123456.root
...
for freeze in range(0,5):
  for i,mass in enumerate(mass_points):
    window=i+1
    mass_path = os.path.join(exe_path, "freezepdf"+str(freeze)+"_cat1/")
