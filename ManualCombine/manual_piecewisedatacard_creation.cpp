...
//write the datacard
void manual_piecewisedatacard_creation(){
  for (int mass=1; mass<12; mass++){
    TString mass_s;
    mass_s = Form("%i_",mass);
    for(int cat=0; cat<3; cat++){
      ...
      cat_n = Form("_%i",cat);
      char inputShape[200];
      sprintf(inputShape,"datacards_piecewise/window"+mass_s+cat_s+".txt");
      ...
      newcardShape << Form("shapes     bkg "+cat_s+" ../ws_piecewise/CMS-HGG_Window"+mass_s+"multipdf_UntaggedTag"+cat_n+".root multipdf:CMS_hgg_UntaggedTag"+cat_n+"_2018_13TeV_bkgshape\n");
      newcardShape << Form("shapes     data_obs "+cat_s+" ../ws_piecewise/CMS-HGG_Window"+mass_s+"multipdf_UntaggedTag"+cat_n+".root multipdf:roohist_data_mass_UntaggedTag"+cat_n+"\n");
