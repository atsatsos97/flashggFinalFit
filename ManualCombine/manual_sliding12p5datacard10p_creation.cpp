...
void manual_sliding12p5datacard10p_creation(){
  for (int mass=10; mass<71; mass++){
    ...
    for(int cat=0; cat<4; cat++){
      ...
      cat_n = Form("_%i",cat);
      char inputShape[200];
      sprintf(inputShape,"datacards_12p5sliding_bigenv/sliding_"+mass_s+cat_s+".txt");
      ...
      newcardShape << Form("shapes     bkg "+cat_s+" ../ws_12p5slidingbigenv/CMS-HGG_"+mass_s+"multipdf_UntaggedTag"+cat_n+".root multipdf:CMS_hgg_UntaggedTag"+cat_n+"_2018_13TeV_bkgshape\n");
      newcardShape << Form("shapes     data_obs "+cat_s+" ../ws_12p5slidingbigenv/CMS-HGG_"+mass_s+"multipdf_UntaggedTag"+cat_n+".root multipdf:roohist_data_mass_UntaggedTag"+cat_n+"\n");
