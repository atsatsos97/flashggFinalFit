...
if opt.batch == 'IC':
  ...
  for itoy in range(0,opt.nToys):
    ...
    gen_cmd = "combine %s -m %.3f -M GenerateOnly --expectSignal=0 --saveWorkspace --toysFrequentist --bypassFrequentistFit -t 1 %s -s -1 -n _%g_gen_step"%(inputWSFile,mh_bf,setParamStr,itoy)
    ...
    fit_cmd = "combine gen_%g.root -m %.3f -M MultiDimFit -P %s --floatOtherPOIs=1 --expectSignal=0 --saveWorkspace --toysFrequentist --bypassFrequentistFit -t 1 %s -s -1 -n _%g_fit_step --cminDefaultMinimizerStrategy 0 --X-rtd MINIMIZER_freezeDisassociatedParams --X-rtd MINIMIZER_multiMin_hideConstants --X-rtd MINIMIZER_multiMin_maskConstraints --X-rtd MINIMIZER_multiMin_maskChannels=2"%(itoy,mh_bf,opt.POIs.split(",")[0],setParamStr,itoy)
    fsub.write("%s\n\n"%fit_cmd)
    # Throw cmd
    fsub.write("#Throw command\n")
    fsub.write("mv higgsCombine_%g_fit_step*.root fit_%g.root\n"%(itoy,itoy))
    throw_cmd = "combine fit_%g.root -m %.3f --snapshotName MultiDimFit -M GenerateOnly --expectSignal=0 --saveToys --saveWorkspace --toysFrequentist --bypassFrequentistFit -t -1 -s -1 -n _%g_throw_step %s"%(itoy,mh_bf,itoy,setParam0Str)
    fsub.write("%s\n\n"%throw_cmd)
    # Clean up
  ...
  gen_cmd = "combine %s -m %.3f -M GenerateOnly --expectSignal=0 --saveWorkspace --saveToys --toysFrequentist --bypassFrequentistFit -t 1 %s -s -1 -n _${itoy}_gen_step"%(inputWSFile,mh_bf,setParamStr) #FIemmi
  if opt.loadSnapshot is not None: gen_cmd += " --snapshotName %s"%opt.loadSnapshot
  fsub.write("%s\n\n"%gen_cmd)
  # Fit cmd
  fsub.write("#Fit command\n")
  ...
  fit_cmd = "combine %s -t 1 --toysFile=gen_${itoy}.root -m %.3f -M MultiDimFit -P %s --floatOtherPOIs=1 --expectSignal=0 --saveWorkspace --toysFrequentist --bypassFrequentistFit %s -n _${itoy}_fit_step --cminDefaultMinimizerStrategy 0 --X-rtd MINIMIZER_freezeDisassociatedParams --X-rtd MINIMIZER_multiMin_hideConstants --X-rtd MINIMIZER_multiMin_maskConstraints --X-rtd MINIMIZER_multiMin_maskChannels=2"%(inputWSFile,mh_bf,opt.POIs.split(",")[0],setParamStr)
  fsub.write("%s\n\n"%fit_cmd)
  # Throw cmd
  fsub.write("#Throw command\n")
  fsub.write("echo Throwing...\n") #FIemmi
  fsub.write("mv higgsCombine_${itoy}_fit_step*.root fit_${itoy}.root\n")
  throw_cmd = "combine fit_${itoy}.root -m %.3f --snapshotName MultiDimFit -M GenerateOnly --expectSignal=0 --saveToys --saveWorkspace --toysFrequentist --bypassFrequentistFit -t -1 -s -1 -n _${itoy}_throw_step %s"%(mh_bf,setParam0Str)
  fsub.write("%s\n\n"%throw_cmd)
  # Clean up
