...
void makeCardsAndWS(TString year="2018"){
  ...
  //INPUT FILE WITH HISTOGRAMS TO FIT BACKGROUND
	  	TFile* file = NULL;
	        ...
	        file=TFile::Open("/afs/cern.ch/work/a/atsatsos/ULLowmassFGG/CMSSW_12_4_8/src/MassPlots_NN0.907/histogramtest_data_"+mass_s+"cat0.root");
  //INPUT FILE WITH HISTOGRAMS TO FIT SIGNAL
	        ...
	        sig_file=TFile::Open("/afs/cern.ch/work/a/atsatsos/ULLowmassFGG/CMSSW_12_4_8/src/MassPlots_NN0.907/histogramtest_sig_"+mass_s+"cat0.root");
  //PREPARE EXPECTED NUMBER OF SIGNAL EVENTS PER CATEGORY
   ...
   //LOOP OVER MASS INDICES AND MAKE THE CARDS/WORKSPACES
		...
		for(int i=0; i<1; i++){
			...
			c_all.SaveAs(Form("output/NN0.907DCB/catA_%d_"+year+"_sig1bkg3_M"+mass_s+"cat0.png",i));
			...
			sig_frame->SetTitle("Signal Fit");
                	c_sig.SaveAs(Form("output/NN0.907DCB/cat_sig_%d_"+year+"_sig1bkg3_M"+mass_s+"cat0.png",i));
			...
			dpworkspace.writeToFile(Form("output/NN0.907DCB/dpWorkspace"+year+suff+"_%d_sig1bkg3_M"+mass_s+"cat0.root",i));

			//write the datacard
			char inputShape[200];
			sprintf(inputShape,"output/NN0.907DCB/dpCard_"+year+suff+"_%d_sig1bkg3_"+mass_s+"cat0.txt",i);
